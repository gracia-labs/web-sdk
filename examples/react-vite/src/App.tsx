import {
    ...
    useGraciaPlaylist,
    XROverlay,
} from "@gracia/web-sdk/core";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import * as THREE from "three";

// Demo content — replace with your streaming IDs and tokens
...
export default function App() {
    ...
    // README #12: XR overlay needs THREE + optional R3F peer deps (see package.json)
    ...
    const showLoader = !gracia.isContentReady && !gracia.error && gracia.isInitialized;
    ...
    return (
        <div style={loaderOverlayStyle}>
            <div style={loaderContainerStyle}>
                ...
                {progress > 0 && <span style={loaderTextStyle}>{Math.round(progress)}%</span>}
            </div>
        </div>
    );
}
            ...
            <div style={modeGroupStyle}>
                ...
                {xr.arSupported && (
                    <button
                        ...
                        onClick={() => xr.setMode(gracia.mode === "ar" ? "pw" : "ar")}
                    >
                ...
                <>
                    ...
                    <div style={sceneNavStyle}>
                        ...
                        <span style={{ fontSize: 12, opacity: 0.5 }}>
                            {playlist.total > 0 ? `${playlist.index + 1} / ${playlist.total}` : "—"}
                        </span>
                        <button
                            style={navBtnStyle}
...
function BottomBar({
    ...
    const playIcon = playback.isBuffering ? (
        <IconSpinner />
    ) : playback.isPlaying ? (
        <IconPause />
    ) : (
    ...
    return (
        <div style={bottomBarStyle}>
            {gracia.error && <div style={errorBannerStyle}>{gracia.error.message}</div>}
...
async function loadDemoSources(playlist: ReturnType<typeof useGraciaPlaylist>) {
    try {
